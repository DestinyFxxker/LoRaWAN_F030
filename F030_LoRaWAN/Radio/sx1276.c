...
void gByteWritefunc(uint8_t buffer)   //写一个字节
{
    //HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_RESET);//set NSS(PA_4) to low
    HAL_SPI_Transmit(&hspi1, &buffer, 1, 10);///&是否应该为*
}

uint8_t gByteReadfunc(void)   //读一个字节
{
    uint8_t temp=0;
...
void SX1276WriteFifo( uint8_t *buffer, uint8_t size )
{
   int ASM_i=0;
    ...
    gByteWritefunc( 0x80 );
	for( ASM_i = 0; ASM_i < size; ASM_i++ )
	{
		gByteWritefunc( *buffer );buffer++;
	}
...
void SX1276ReadFifo( uint8_t *buffer, uint8_t size )
{
	...
	gByteWritefunc( 0x00 );
	for(RF_REC_RLEN_i=0;RF_REC_RLEN_i<size;RF_REC_RLEN_i++)
	{
		buffer[RF_REC_RLEN_i]=gByteReadfunc();
	}
