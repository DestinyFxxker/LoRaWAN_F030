 ...
 */
//Ëøô‰∏§‰∏™ÁßòÈí•‰∏çÊòØËÆ°ÁÆóÂá∫Êù•ÁöÑ‰πàÔºü
...
//LoRaMac-definitions.hŒƒº˛140––”–œÍœ∏Àµ√˜
/*!
...
static void OnRadioTxDone( void )
{
    ...
    // Setup timers
    //‰∏ãË°åÊï∞ÊçÆÁöÑÂºÄÂßãÔºåÊòØÂú®ÂèëÈÄÅÂÆåÊàêÂêéÔºåÂª∂Êó∂‰∏ÄÊÆµÊó∂Èó¥ÔºåÂàÜÂà´ÂºÄÂêØRX1ÂíåRX2Êé•Êî∂Á™óÂè£
        ...
        case FRAME_TYPE_DATA_UNCONFIRMED_DOWN:
            {
                ...
                if( isMicOk == true )
                {
                    ...
                    if( ( ( size - 4 ) - appPayloadStartIndex ) > 0 )
                    {
                        ...
                        //MACÂëΩ‰ª§ÔºåË¶Å‰πà‰ΩøÁî®Fport0Êù•ÂçïÁã¨‰º†ËæìÔºåË¶Å‰πà‰ΩøÁî®Èù?ÁöÑFPortÊù•ÂíåÊï∞ÊçÆ‰∏ÄËµ∑‰º†Ëæ?
                        if( port == 0 )
                        {
                        ...
                        else//ËøôÈáåÂ∞±ÊòØË¶Å‰πà‰ΩøÁî®Èù?ÁöÑFPortÊù•ÂíåÊï∞ÊçÆ‰∏ÄËµ∑‰º†Ëæ?
                        {
                            if( fCtrl.Bits.FOptsLen > 0 )
                            {
                    ...
                    else
                    {
                        if( fCtrl.Bits.FOptsLen > 0 )
                        {
                            // Decode Options field MAC commands MACÂ∏ßËøõÊù•‰ª•ÂêéÔºåÈÄöËøáÂ±ÇÂ±ÇÁ≠õÈÄâÔºåÊúÄÁªàÂà∞ËææËøô‰∏™ÂáΩÊï∞Êù•Â§ÑÁêÜMACÂëΩ‰ª§
...
static void OnRxWindow1TimerEvent( void )
{
    ...
    //Âà©Áî® ( Channel % 48 ) Â∞Ü‰∏äË°å‰ø°ÈÅìÂàÜ‰∏?~7ÔºåÂàÜÂà´‰∏é‰∏ãË°å‰ø°ÈÅìÁö?~7ÂØπÂ∫î
#elif defined( USE_BAND_470 ) || defined( USE_BAND_470PREQUEL )
...
#elif defined( USE_BAND_470 ) || defined ( USE_BAND_470PREQUEL )
//µ´ «°≠°≠1301/1308Õ¯πÿ–æ∆¨÷ª”–8∏ˆ…œ–––≈µ¿£¨À˘“‘–Ë“™…Ë÷√SDKµƒ–≈µ¿—⁄¬Î£¨÷ªø™∆Ù–Ë“™µƒ–≈µ¿
    LoRaMacParamsDefaults.ChannelsMask[0] = 0x00FF;//bit 15~0,±Ì æ±æ¿˝◊”÷ª π”√0~7–≈µ¿
...
LoRaMacStatus_t LoRaMacMibGetRequestConfirm( MibRequestConfirm_t *mibGet )
{
    ...
    DebugPrintf("LoRaMacStatus_t status = LORAMAC_STATUS_OK;\r\n");
    if( mibGet == NULL )
    {
        return LORAMAC_STATUS_PARAMETER_INVALID;
    }
...
LoRaMacStatus_t LoRaMacMlmeRequest( MlmeReq_t *mlmeRequest )
{
    ...
    if( status != LORAMAC_STATUS_OK )
    {
				DebugPrintf("status != LORAMAC_STATUS_OK\r\n");
        NodeAckRequested = false;
        LoRaMacFlags.Bits.MlmeReq = 0;
    }
