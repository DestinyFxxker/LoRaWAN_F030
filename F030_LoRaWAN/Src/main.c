 ...
 */
static uint8_t AppData[LORAWAN_APP_DATA_MAX_SIZE];//要发送的数据
...
//					
			      ...
			      //ADXL357ReadOverall(AppData);
						//SHT3XReadOverall(AppData);
						UartPutBuffer(&Uart1,(uint8_t *)"+ReadOverall\r\n",strlen("+ReadOverall\r\n"));
...
static void McpsIndication( McpsIndication_t *mcpsIndication )
{
    ...
    { 
      UartPutBuffer(&Uart1,(uint8_t *)"+SEND:ACK RECEIVED\r\n",strlen("+SEND:ACK RECEIVED\r\n"));
      UartPutBuffer(&Uart1,(uint8_t *)"+SEND:DONE\r\n\r\n",strlen("+SEND:DONE\r\n\r\n"));
    }
...
//							  TimerStart(&iWDG_FEED);  
            ...
            case DEVICE_STATE_JOIN:
            {
                ...
                if( NextTx == true )
                {
                    UartPutBuffer(&Uart1,(uint8_t *)"+JOIN:STARTING...\r\n",strlen("+JOIN:STARTING...\r\n"));
                    LoRaMacMlmeRequest( &mlmeReq );
                }
                DeviceState = DEVICE_STATE_SLEEP;
#else
            ...
            case DEVICE_STATE_SEND: //周期发送数据
            {
							UartPutBuffer(&Uart1,(uint8_t *)"Into send state\r\n",strlen("Into send state\r\n"));
							if( NextTx == true )
							{
								UartPutBuffer(&Uart1,(uint8_t *)"In sending...\r\n",strlen("In sending...\r\n"));
								BoardInitPeriph( );
								//HAL_Delay(2000);
								PrepareTxFrame( AppPort );//发送周期通过这个函数实现
								
								NextTx = SendFrame( );//发送数据的函数
							}
							if( ComplianceTest.Running == true )
							{
							...
							DeviceState = DEVICE_STATE_CYCLE;//发送结束后……进入低功耗状态
							break;
            }
            case DEVICE_STATE_CYCLE:
            {
                DeviceState = DEVICE_STATE_SLEEP;//低功耗函数
            ...
            default:
            {
                DeviceState = DEVICE_STATE_INIT;//当需要再次发送时，再次进入初始化状态，重复以上周期
                break;
            }
        }
    }
