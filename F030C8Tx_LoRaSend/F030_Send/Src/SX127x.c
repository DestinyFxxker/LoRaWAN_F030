...
unsigned char   Frequency[3]={0x6C,0x40,0x12};//
unsigned char   SpreadingFactor=12;    //7-12,扩频因子选小一些，发射时间会更快一些。
unsigned char   CodingRate=2;        //1-4
unsigned char   Bw_Frequency=7;      //6-9
unsigned char   powerValue= 20;       //功率设置，调试的时候可以先设置小一点
...
{
	HAL_GPIO_WritePin(GPIOB, GPIO_PIN_1, GPIO_PIN_RESET);//set LoRa RESET(PA_1) to low
	HAL_Delay(200);
...
void SX127x_Interupt(void)
{
	...
	if((RF_EX0_STATUS&0x40)==0x40)  //接收完成
	{
		...
		if((CRC_Value&0x04) == 0x04) //是否打开CRC校验
		{
			...
			for(RF_REC_RLEN_i=0;RF_REC_RLEN_i<SX127x_RLEN;RF_REC_RLEN_i++)
			{
				gByteWritefunc(temp0[RF_REC_RLEN_i]);
			}
			HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_SET); //取消选中SX127x
			//fqcRecvData(recv);  //处理下接收到的数据
			//DebugPrintf("REG_LR_IRQFLAGS:%x\r\n",SX127xReadBuffer( REG_LR_IRQFLAGS ));		

			
			//DebugPrintf("REG_LR_IRQFLAGSMASK:%x\r\n",SX127xReadBuffer( REG_LR_IRQFLAGSMASK ));	
			//DebugPrintf("REG_LR_IRQFLAGS:%x\r\n",SX127xReadBuffer( REG_LR_IRQFLAGS ));	
					
					
					FUN_RF_SENDPACKET((unsigned char *)recv,SX127x_RLEN);	//lora发送数据
					
					while((SX127xReadBuffer( REG_LR_IRQFLAGS ) & 0x08) != 0x08)
					{
						HAL_Delay(10);
						//DebugPrintf("Irq_flag：%x\r\n",Irq_flag);
					}
					DebugPrintf("lora发送数据成功：%s,长度%d.\r\n\r\n",recv,SX127x_RLEN);
					memset(recv, 0, sizeof(recv));//清空USART_RX_BUF
